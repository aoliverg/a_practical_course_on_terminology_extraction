...
class TBXTools:
    ...
    def load_tosearch_terms(self,SLterms,encoding="utf-8",nmin=0,nmax=1000,compoundify=False):
    ...
    def prepare_find_translation_comparable_corpus(self,mapping_dictionary="MUSE-en-es.txt"):
        ...
        self.mapEmbeddings("embeddingsSL.emb","embeddingsTL.emb","mappedembeddingsSL.emb","mappedembeddingsTL.emb",mapping_dictionary)        
        
            
    def find_translation_comparable_corpus(self,mapping_dictionary="MUSE-en-es.txt",maxdec=1,maxinc=2,candidates=25):
        tofind=[]
        result={}
        self.cur.execute("SELECT term FROM tosearch_terms")
        for s in self.cur.fetchall():
            tofind.append(s[0])
        self.load_SL_embeddings("mappedembeddingsSL.emb")
        self.load_TL_embeddings("mappedembeddingsTL.emb")
        stopwords=[]
        with self.conn:
            self.cur.execute("SELECT tl_stopword FROM tl_stopwords")
            for s in self.cur.fetchall():
    ...
    def load_POS_model_spacy_from_file(self, model):
        self.POSmodel_spacy=spacy.load(model)
